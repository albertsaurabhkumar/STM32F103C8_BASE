...
TXBn txBuffers[3] = {TXB0, TXB1, TXB2};
...
ERROR readMsg(const RXBn rxbn, struct can_frame *frame)
{
...
ERROR readMessage(struct can_frame *frame)
{
    ...
    if ( stat & STAT_RX0IF ) {
        rc = readMsg(RXB0, frame);
    } else if ( stat & STAT_RX1IF ) {
        rc = readMsg(RXB1, frame);
    } else {
        rc = ERROR_NOMSG;
    }
